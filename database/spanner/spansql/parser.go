...
func ParseDDLAndDML(filename, s string) (*DDLAndDML, error) {
	...
	for {
		...
		ddlStmt, err := p.parseDDLStmt()
		if err != nil && err != unknownDDL {
			return nil, err
		}
		if err == nil {
			stmt := DDLOrDMLStmt{
			...
			ddlAndDml.List = append(ddlAndDml.List, stmt)
		} else if err == unknownDDL {
			dmlStmt, err := p.parseDMLStmt()
			if err != nil {
				return nil, err
			}
			stmt := DDLOrDMLStmt{
	...
	for _, com := range p.comments {
		...
		ddlAndDml.Comments = append(ddlAndDml.Comments, c)
	}

	return ddlAndDml, nil
}
...
var unknownDDL = &parseError{message: "unknown DDL statement"}
