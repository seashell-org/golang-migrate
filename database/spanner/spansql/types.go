...
type DDLOrDMLStmt struct {
	T   string
	Ddl DDLStmt
	Dml DMLStmt
}

func (d *DDLAndDML) clearOffset() {
	for _, stmt := range d.List {
		if stmt.T == ddlType {
			stmt.Ddl.clearOffset()
		}
	}
...
func (ddl *DDLAndDML) LeadingComment(n Node) *Comment {
...
func (ddl *DDLAndDML) InlineComment(n Node) *Comment {
